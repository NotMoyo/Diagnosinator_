...
class Main_Window(qtw.QMainWindow):
    ...
    def build_main_window_toolbar(self):
        ...
        self.main_window_toolbar.addAction("Edit Patient")
        self.main_window_toolbar.addAction("Delete Patient", self.delete_patient)
    ...
    def build_main_window_patients_table(self):
        ...
        #
        self.main_window_patients_table_horizontal_headers = ["MRN", "Last Name", "First Name", "Date of Birth"]
        self.main_window_patients_table.setColumnCount(len(self.main_window_patients_table_horizontal_headers))
        self.main_window_patients_table.setHorizontalHeaderLabels(self.main_window_patients_table_horizontal_headers)
        #
        ...
        self.populate_main_window_patients_table()
    ...
    #
    # @benchmark
    def populate_main_window_patients_table(self):
        #
        self.main_window_patients_table.clearContents()
        self.main_window_patients_table.setRowCount(0)
        #
        patients = self.PT_DB.load_patients_list()
        #
        for patient in patients:
            self.main_window_patients_table.insertRow(self.main_window_patients_table.rowCount())
            for column_index, column in enumerate(patient):
                self.main_window_patients_table.setItem(self.main_window_patients_table.rowCount() - 1, column_index, qtw.QTableWidgetItem(str(column)))
    #
    @benchmark
    ...
    def delete_patient(self):
        #
        selected_rows = self.main_window_patients_table.selectionModel().selectedRows()
        if len(selected_rows) == 0:
            return
        #
        for row in selected_rows:
            mrn = self.main_window_patients_table.item(row.row(), 0).text()
            self.PT_DB.delete_patient(mrn)
        #
        self.populate_main_window_patients_table()
...
class Patient_Add_Window(qtw.QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.build_patient_add_window()
    #
    @benchmark
    def build_patient_add_window(self):
        ...
        #
        for field in self.patient_add_window_fields.values():
            if isinstance(field, qtw.QLineEdit):
                field.textChanged.connect(lambda text, widget=field: widget.setText(text.upper()))
        #
        ...
        self.patient_add_window_fields["save_button"].clicked.connect(mw.populate_main_window_patients_table)
    ...
    def save_patient(self, patient_add_window_fields):
        ...
        #
        for name, field in patient_add_window_fields.items():
            if name == "mrn_input":
                field.setText(str(mw.PT_DB.find_next_available_mrn()))
            elif isinstance(field, qtw.QLineEdit):
                field.clear()
            elif isinstance(field, qtw.QComboBox):
